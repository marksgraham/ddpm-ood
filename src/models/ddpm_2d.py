...
class DDPM(nn.Module):
    def __init__(
        ...
        self.vlb_loss_weighting = vlb_loss_weighting
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, return_x0=False):
        ...
        # need to split out the mean and variance predictions
        if self.learn_predicted_variance:
            B, C = x.shape[:2]
            assert model_out.shape == (B, C * 2, *x.shape[2:])
            # direct prediction of variance
            # model_out, model_log_variance= torch.split(model_out, C, dim=1)
        ...
        if self.learn_predicted_variance:
            ...
            posterior_log_variance = model_log_variance
    ...
    def p_losses(self, x_start, t, weights, noise=None, crop_function=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.model(x_noisy, t)
        if crop_function:
            x_start = crop_function(x_start)
            x_noisy = crop_function(x_noisy)
            model_output = crop_function(model_output)
            noise = crop_function(noise)
        # need to split out the mean and variance predictions
        if self.learn_predicted_variance:
            B, C = x_noisy.shape[:2]
        ...
        # my code to learn the variance too
        if self.learn_predicted_variance:
        ...
        # # this code was already here but doesn't actually seem to learn the VLB; it just calculated the MSE again..
        # else:

...
def parse_args():
    ...
    parser.add_argument("--max_t", type=int, default=1000, help="Maximum T to consider reconstructions from.")
...
def main(args):
    ...
    total_steps = 0
    full_timesteps = make_ddim_timesteps(num_ddim_timesteps=100, num_ddpm_timesteps=1000, ddim_discr_method='uniform')
    ...
    print(f'SETTING MAX_T to {MAX_T} and T_SKIP to {T_SKIP_FACTOR} with a total of'
    ...
    if plot_target == 'mse+perceptual':
        results_df_val_pivot = results_df_val.pivot_table(index=['filename'], columns=['t'], values=['mse','perceptual_difference'])
    else:
        results_df_val_pivot = results_df_val.pivot_table(index=['filename'], columns=['t'], values=[plot_target])
    ...
    for out_dataset in out_data:
        ...
        for target in ['perceptual_difference','mse']:
            ...
            results_df_val_agg = results_df_val.groupby(['t']).agg({target: ['mean', 'std']})[target].reset_index().rename({'mean': f'val_mean_{target}', 'std': f'val_std_{target}'}, axis=1)
            results_df = results_df.merge(results_df_val_agg,on=['t'],how='left')
            results_df[f'z_score_{target}'] = (results_df[target] - results_df[f'val_mean_{target}']) / results_df[f'val_std_{target}']
        ...
        if plot_target == 'mse+perceptual':
            results_df['z_score_mse+perceptual']=results_df['z_score_mse'] + results_df['z_score_perceptual_difference']

        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(dataloader, total=len(dataloader))):
                ...
                for idx, t in enumerate(t_vals):
                    ...
                    latent_denoised, _ = diffusion_plms.sample(
                    ...
                    for b in range(batch_size):
                        if args.config_vqvae_file == "None":
                            if x_t_upper.shape[-2:][0] < 32:
                                ...
                                perceptual_difference_spatial = pl_spatial(
                                    torchpad(image[b, None, ...].cpu(), (2, 2, 2, 2)),
                                    torchpad(
                                        latent_denoised[b, None, ...].clamp(0, 1).cpu(),
                                        (2, 2, 2, 2),
                                    ),
                                ).numpy()
